...
def getRandomNodes(G: nx.Graph, k) -> int:
...
def getRandomNodesFromLabels(G, k, flag):
...
def getNodesFromLabelsWithHighestDegree(G, k, flag):
    random_nodes = {}
    dict_degrees = {}
    for node in G.nodes():
        dict_degrees[node] = G.degree(node)
    sorted_dict = sorted(dict_degrees.items(), key=itemgetter(
...
def performRandomWalkFull(G, starting_node, user_nodes):
    ...
    while(flag != 1):
        ...
        if(user_nodes.has_key(starting_node)):
            ...
            print >> sys.stderr, "seen nodes ", len(
                dict_already_seen_nodes.keys())
...
if __name__ == "__main__":
	if len(sys.argv) < 4:
		filename = 'political_blogs_largest_CC.txt'
		file2 = sys.argv[2]  # ! fixme
		percent = 0.1
	else:
		filename = sys.argv[1]
		file2 = sys.argv[2]
    ...
    # print "randomly selected user nodes ", user_nodes
        ...
        for i in range(len(user_nodes_left_list)-1):
            ...
            side = performRandomWalk(
                G, node, other_nodes_dict, user_nodes_right)
            print(side)
            if(side == "left"):
                left_left += 1
            elif(side == "right"):
        ...
        for i in range(len(user_nodes_right_list)-1):
            ...
            other_nodes = user_nodes_right_list[:i] + \
                user_nodes_right_list[i+1:]
            other_nodes_dict = getDict(other_nodes)
            side = performRandomWalk(
                G, node, user_nodes_left, other_nodes_dict)
            if(side == "left"):
                right_left += 1
            elif(side == "right"):
                right_right += 1
