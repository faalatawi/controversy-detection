...
if __name__ == "__main__":
    ...
    G = nx.read_weighted_edgelist(
        filename, delimiter=',', create_using=nx.DiGraph())

    G = convertIntoDirected(G)
    # seed nodes percenatage = 5 percent
    seed_nodes_num = int(len(G.nodes())*0.05)
    #seed_nodes_num = 1;
    ...
    left_seed_nodes = getNodesFromLabelsWithHighestDegree(
        G, seed_nodes_num, 'left')
    right_seed_nodes = getNodesFromLabelsWithHighestDegree(
        G, seed_nodes_num, 'right')
